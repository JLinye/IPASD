...
class ResNet(nn.Module):
    ...
    def forward(self, x, y=None, loss_type='cross_entropy', feature=False, embedding=False):
        ...
        x = self.conv1(x)   # B x 64 x 112 x 112
        x = self.bn1(x)     # B x 64 x 112 x 112
        x = self.relu(x)    # B x 64 x 112 x 112
        x = self.maxpool(x) # B x 64 x 56  x 56

        x = self.layer1(x)
        f1 = x              # B x 64 x 56 x 56
        x = self.layer2(x)
        f2 = x              # B x 128 x 28 x 28
        x = self.layer3(x)
        f3 = x              # B x 256 x 14 x 14
        x = self.layer4(x)
        f4 = x              # B x 512 x 7 x 7
        

        x = self.avgpool(x) # B x 512 x 1 x 1
        x = x.view(x.size(0), -1)
        embedding0 = x      # B x 512
        logits = self.fc(x)
...
if __name__ == '__main__':
    net = resnet18_imagenet(num_classes=1000)
